...
const LocationService_1 = __importDefault(require("./core/LocationService"));
const server_1 = require("./server");
commander_1.program
    .addArgument(new commander_1.Argument('[input_file]', 'File containing the locations (.csv or .txt)'))
    .addOption(new commander_1.Option('-s, --server', 'Starts the server mode'))
    .addOption(new commander_1.Option('--port [number]', 'Port to start the server').env('PORT').default(3000))
    ...
    const service = new LocationService_1.default(options.keys ? yield (0, files_1.readLines)(options.keys) : options.key ? [options.key] : []);
    if (options.server) {
        consola_1.default.debug('Starting server ...');
        const server = (0, server_1.app)(service).listen(options.port, () => consola_1.default.info(`Server started at port :${options.port}`));
        process.on('SIGINT', () => server.close());
        return new Promise((resolve, reject) => {
            server.on('close', (err) => (err ? reject(err) : resolve()));
        });
    }
